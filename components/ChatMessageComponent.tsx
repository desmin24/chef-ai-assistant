...
export const ChatMessageComponent: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const getIcon = () => {
    switch (role) {
      case MessageRole.USER:
        return <i className="fas fa-user-circle text-xl mr-2 text-blue-100"></i>;
      case MessageRole.MODEL:
        return <i className="fas fa-robot text-xl mr-2 text-green-100"></i>;
      case MessageRole.SYSTEM:
         return <i className="fas fa-info-circle text-base mr-2 text-gray-500"></i>;
      default:
        return null;
    }
  }
  ...
  if (role === MessageRole.SYSTEM && !text.startsWith("發生錯誤：")) {
     return (
      <div className={`${baseClasses} ${getRoleSpecificStyling()} mx-auto w-full sm:w-auto`}>
  ...
  if (role === MessageRole.SYSTEM && text.startsWith("發生錯誤：")) {
     return (
      <div className={`${baseClasses} bg-red-100 text-red-700 self-center text-center italic text-sm rounded-xl mx-auto w-full sm:w-auto`}>
        <div className="flex items-center justify-center">
         <i className="fas fa-exclamation-triangle text-base mr-2 text-red-500"></i>
         <p className="text-xs sm:text-sm">{text}</p>
        </div>
      </div>
     );
  ...
  return (
    <div className={`flex ${role === MessageRole.USER ? 'justify-end' : 'justify-start'}`}>
